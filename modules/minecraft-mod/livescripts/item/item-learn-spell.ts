    ...
    for (let i = 0; i < itemIDs.length; i++) {
        events.ItemID.OnCanEquip(itemIDs[i], (item, player, result) => {
            ...
            player.SendBroadcastMessage(
                "You feel a surge of energy enter your mind. You have learned how to use |Hspell:" +
                itemIDToSpellID[entry] +
                "|h|r|cff71d5ff[" +
                itemIDToSpellName[entry] +
                "]|r|h"
            );
        });
        events.ItemID.OnUnequip(itemIDs[i], (item, player, isSwap, result) => {
            ...
            player.SendBroadcastMessage(
                "A draining feeling washes over you and you lose the ability to use |Hspell:" +
                itemIDToSpellID[entry] +
                "|h|r|cff71d5ff[" +
                itemIDToSpellName[entry] +
                "]|r|h"
