...
export function dungeon1(events: TSEventHandlers) {
    ...
    events.GameObjectID.OnGossipSelect(
        GetID("gameobject_template", "minecraft-mod", "torghastendobj"),
        (obj, player, menuID, sel, cancel) => {
            if (sel == 0) {
                let mapChoice = getRandomInt(mobSpawnCoords.length);
                resetGroup(
                    player,
                    playerSpawnCoords[mapChoice],
                    bossSpawnCoords[mapChoice],
                    bossIDs,
                    mobSpawnCoords[mapChoice],
                    mobIDs,
                    vendorSpawnCoords[mapChoice]
                );
                obj.Despawn();
            } else if (sel == 1) {
                rewardGroup(player);
    ...
    events.MapID.OnPlayerLeave(726, (map, player) => {
        ...
        if (rewCount > 0) {
            player.SendAreaTriggerMessage(
                "it seems you did not fare so well, have " +
                rewCount +
                " Anima for your attempt."
            );
