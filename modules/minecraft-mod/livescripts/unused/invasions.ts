...
export function invasions(events:TSEventHandlers){
    ...
    events.CreatureID.OnDeath(GetID("creature_template","minecraft-mod","invasion-minion-01"),(creature,killer)=>{
        addPercent(0.5)
    })
    events.CreatureID.OnDeath(GetID("creature_template","minecraft-mod","invasion-largeminion-01"),(creature,killer)=>{
        addPercent(1)
    })
    events.CreatureID.OnDeath(GetID("creature_template","minecraft-mod","invasion-largeminion-02"),(creature,killer)=>{
        addPercent(3)
    })
    events.CreatureID.OnDeath(GetID("creature_template","minecraft-mod","invasion-largeminion-03"),(creature,killer)=>{
        addPercent(5)
    })
...
function startInvasion(){
    ...
    let q = QueryWorld('SELECT * FROM `invasions` WHERE eventID='+currentEventID)
    while(q.GetRow()){
        if(q.GetInt32(1) == 1){//creature
            let unit = spawner.SpawnCreature(q.GetInt32(2),q.GetInt32(3),q.GetInt32(4),q.GetInt32(5),q.GetInt32(6),8,0)
            cSpawns.push(unit)
        }else if(q.GetInt32(1) == 0){//gameobject
...
function checkPercent(){
    if(currentPercent >= 100){
        ...
        while(q.GetRow()){
            let spawner = spawners[currentEventID]
            spawner.SpawnCreature(q.GetInt32(1),spawner.GetX(),spawner.GetY(),spawner.GetZ(),spawner.GetO(),8,0)
        }
    }
}
...
function addSpawner(spawner:TSPlayer){
    ...
    while(q.GetRow()){
        spawner.SpawnCreature(q.GetInt32(0),q.GetInt32(1),q.GetInt32(2),q.GetInt32(3),q.GetInt32(4),8,0)
