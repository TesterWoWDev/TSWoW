export function onLoginEvents(events:TSEventHandlers){
    events.Player.OnLogin((player,first)=>{
        ...
        SendWorldMessage('|cff'+ colors[player.GetClass()] + "|Hplayer:" + player.GetName() + "|h["+player.GetName() + ']|h|r, the level [' +classIcons[player.GetClass()] + ' ' +player.GetLevel()+ '] '+'|cff'+ colors[player.GetClass()] + player.GetClassAsString(0).toLowerCase() + '|r has logged in.')
        SendWorldMessage('['+classIcons[player.GetClass()] + ' ' +player.GetLevel()+'] '+'|cff'+ colors[player.GetClass()] + "|Hplayer:" + player.GetName() + "|h["+player.GetName() + ']|h' + ' has logged in.')
