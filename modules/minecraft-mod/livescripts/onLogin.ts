export function onLoginEvents(events:TSEventHandlers){
    events.Player.OnLogin((player,first)=>{
        ...
        SendWorldMessage('|cff'+ colors[player.GetClass()] + "|Hplayer:" + player.GetName() + "|h" + ' the level ' +player.GetLevel() + ' ' + player.GetClassAsString(0) + ' has logged in.')
        SendWorldMessage('['+classIcons[player.GetClass()] + '['+player.GetLevel()+']' + player.GetName() + ' has logged in.')
