...
MiniBossBurdenofKnowledge.Effects.get(0).BasePoints.set(649);
MiniBossBurdenofKnowledge.Effects.get(0).DieSides.set(140);
MiniBossBurdenofKnowledge.Effects.get(0).Radius.set(35);
MiniBossBurdenofKnowledge.Effects.get(1).BasePoints.set(-401);
MiniBossBurdenofKnowledge.Effects.get(1).DieSides.set(1);
...
MiniBossTouchofDeathDamage.Effects.get(0).BasePoints.set(199)
MiniBossTouchofDeathDamage.Effects.get(0).DieSides.set(1)
...
MiniBossSummonSpell.Effects.get(0).BasePoints.set(2)
MiniBossSummonSpell.Effects.get(0).DieSides.set(1)
...
TorghastMiniBoss1.InlineScripts.OnJustEnteredCombat((creature, target) => {
    ...
    creature.AddTimer("event2", 13000, -1, (timer, entity, del, can) => {
        let self = entity.ToCreature();
        let target = self.GetVictim();
