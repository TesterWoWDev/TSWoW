...
TorghastBoss5.InlineScripts.OnJustEnteredCombat((creature, target) => {
    const spellID1 = Boss5Energize1.ID
    const spellID2 = Boss5Fulmination1.ID
    const spellID3 = Boss5Magnetize1.ID
    const spellID4 = Boss5Nuke1.ID
    const SpellID5 = Boss5Overload1.ID
    const SpellID6 = Boss5Execution1.ID
    creature.AddTimer('event1', 7000, -1, (timer, entity, del, can) => {
        let self = entity.ToCreature()
        let target = self.GetVictim()   
        self.CastSpell(target, spellID1, false)                 // Energize Dot
    })
    creature.AddTimer('event2', 12000, -1, (timer, entity, del, can) => {
        let self = entity.ToCreature()
        let target = self.GetVictim()
        self.CastSpell(target, spellID2, false)                 //Fulmination Buff/Dot
    })
    creature.AddTimer('event3', 10000, -1, (timer, entity, del, can) => {
        let self = entity.ToCreature()
        let target = self.GetVictim()
        self.CastSpell(target, spellID3, false)                 //Magnetize Mass Pull
        creature.AddTimer('event3.1', 2000, 0, (timer, entity, del, can) => {
            let self = entity.ToCreature()
            let target = self.GetVictim()
            self.CastSpell(target, spellID4, false)             //Danger Zone Mass AOE (Interruptable)
        })
    creature.AddTimer('event4', 25000, -1, (timer, entity, del, can) => {
            let self = entity.ToCreature()
            let target = self.GetVictim()
            self.CastSpell(target, SpellID5, false)             //Overload Mass AOE (Uninterruptable)
        })
    creature.AddTimer('event5', 32500, -1, (timer, entity, del, can) => {
            let self = entity.ToCreature()
            let target = self.GetVictim()
            self.CastSpell(target, SpellID6, false)             //Execution 1 Shot (Interruptable)
        })
    })
})
