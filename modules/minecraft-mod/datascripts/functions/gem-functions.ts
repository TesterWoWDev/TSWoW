...
export function createGem(
    ...
    gem.DisplayInfo.modRefCopy((value) => displayinfo);
...
export function gemLevelUpPattern(
    dustID: number,
    startingGemID: number[],
    endingGemID: number[]
) {
    startingGemID.forEach((value, index, array) => {
        createRecipe(
            [0, dustID, 0, dustID, value, dustID, 0, dustID, 0],
            [value, dustID, 0, 0, 0, 0, 0, 0, 0],
            [1, 4, 0, 0, 0, 0, 0, 0, 0],
            endingGemID[index],
            1
        );
...
export function createAllUndiscoverGems(
    gemCount: number,
    gemName: string,
    gemDisplayID: any
): number[] {
    ...
    for (let i = 0; i < gemCount; i++) {
        let gem = std.Items.create(
            MODNAME,
            gemName.toLowerCase().replace(" ", "-") + "-" + i,
            2934
        );
        gem.Name.enGB.set("Unrefined " + gemName);
        gem.DisplayInfo.modRefCopy((value) => gemDisplayID);
        gem.Material.set(8);
