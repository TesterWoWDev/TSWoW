...
export function makeResourceNode(name:string,displayID:number,lockID:number,modPrefix:string):GameObjectChest{
    let resourceNode = std.GameObjectTemplates.Chests.create(MODNAME,modPrefix + name.toLowerCase().replace(' ','-'))
...
export function makePool(poolEntry:number, node: GameObjectChest, maxLimit:number, respawnRate:number, gobPositions:Position[]){
    SQL.pool_template.add(poolEntry).max_limit.set(maxLimit)
    gobPositions.forEach((element,index) => {
        node.Spawns.addMod(MODNAME,poolEntry+'spawn'+index,gobPositions,instance=>{
            instance.SpawnTimeSecs.set(respawnRate)
            SQL.pool_members.add(1,instance.row.guid.get()).poolSpawnId.set(poolEntry).chance.set(100)
        })
