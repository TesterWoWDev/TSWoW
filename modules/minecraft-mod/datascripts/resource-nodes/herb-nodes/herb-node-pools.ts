// import { retardMaudan, spawn1Tier1, spawn2Tier1, spawn2Tier2, spawn3Tier1, spawn3Tier2, spawn3Tier3, spawn4Tier1, spawn4Tier2, spawn4Tier3, spawn4Tier4 } from "./herb-node-spawns";
// import { makePool } from "../../functions/resource-node-functions";
// import { node1, node2, node3, node4 } from "./herb-nodes";

// let poolEntry = 100100
// let respawnRate = 300
//tier 1 area
makePool(poolEntry++, node1, 30, respawnRate, spawn1Tier1)

//tier2 area
makePool(poolEntry++, node1, 5, respawnRate, spawn2Tier1)
makePool(poolEntry++, node2, 25, respawnRate, spawn2Tier2)

//tier3 area
makePool(poolEntry++, node1, 5, respawnRate, spawn3Tier1)
makePool(poolEntry++, node2, 10, respawnRate, spawn3Tier2)
makePool(poolEntry++, node3, 20, respawnRate, spawn3Tier3)

//tier4 area
makePool(poolEntry++, node1, 3, respawnRate, spawn4Tier1)
makePool(poolEntry++, node2, 5, respawnRate, spawn4Tier2)
makePool(poolEntry++, node3, 5, respawnRate, spawn4Tier3)
makePool(poolEntry++, node4, 15, respawnRate, spawn4Tier4)