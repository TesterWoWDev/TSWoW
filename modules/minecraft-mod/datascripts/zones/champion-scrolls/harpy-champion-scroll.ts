...
export let harpyCreature = std.CreatureTemplates.create(
    MODNAME,
    "harpycreature-creature",
    299
);
...
harpyCreature.Scripts.onUpdateIc(0, 0, 0, 0, (script) => {
...
harpyCreature.Scripts.onUpdateOoc(0, 0, 0, 0, (script) => {
    script.row.event_flags.set(1);
    script.Action.setRemoveTimedEvent(0);
    script.Action.setRemoveTimedEvent(1);
    script.Action.setRemoveTimedEvent(2);
});
harpyCreature.Scripts.onTimedEventTriggered(0, (script) => {
    script.Target.setVictim();
    script.Action.setCast(harpySpell1.ID, 2, 7);
});
harpyCreature.Scripts.onTimedEventTriggered(1, (script) => {
    script.Target.setVictim();
    script.Action.setCast(harpySpell2.ID, 2, 7);
});
harpyCreature.Scripts.onTimedEventTriggered(2, (script) => {
    script.Target.setVictim();
    script.Action.setCast(harpySpell3.ID, 2, 7);
});
//End of Spells
harpyCreature.FactionTemplate.set(48);
harpyCreature.DamageSchool.Normal.set();
...
harpyScroll.Description.enGB.set(
    "This scroll contains the ability to summon a powerful Harpy, be weary, it is advised that this challenge be accepted with additional party members."
);
...
export let harpyCreatureSpawn = std.CreatureTemplates.create(
    MODNAME,
    "harpycreaturespawn-creature",
    8776
);
...
export let chanceharpy1 = std.Spells.create(
    MODNAME,
    "chanceharpy1-spell",
    13049
); //15283
...
export let chanceharpy1m = std.Spells.create(
    MODNAME,
    "chanceharpy1m-spell",
    67670
); //melee
chanceharpy1m.Name.enGB.set("Summon Harpy");
chanceharpy1m.Description.enGB.set(
    "Your attacks and abilties have the chance to summon a ravaging Harpy to fight for you."
);
chanceharpy1m.AuraDescription.enGB.set(
    "Your attacks and abilties have the chance to summon a ravaging Harpy to fight for you."
);
chanceharpy1m.Effects.get(0).TriggerSpell.set(chanceharpy1.ID);
export let chanceharpy1c = std.Spells.create(
    MODNAME,
    "chanceharpy1c-spell",
    67672
); //caster
chanceharpy1c.Name.enGB.set("Summon Harpy");
chanceharpy1c.Description.enGB.set(
    "Your attacks and abilties have the chance to summon a ravaging Harpy to fight for you."
);
chanceharpy1c.AuraDescription.enGB.set(
    "Your attacks and abilties have the chance to summon a ravaging Harpy to fight for you."
);
