...
//Testing Zone
export let Consecrationdamage = std.Spells.create(
    MODNAME,
    "Consecrationdamage-spell",
    20116
);

export let ThunderClapModifier = std.Spells.create(
    MODNAME,
    "thunderclaptest-spell",
    58098
); //caster
ThunderClapModifier.Name.enGB.set("Thunder Clap (Effect)");
ThunderClapModifier.Description.enGB.set(
    "Thunder Clap now leaves behind consecration."
);
ThunderClapModifier.AuraDescription.enGB.set(
    "Your attacks and abilties now have the chance to leave behind consecration."
);
ThunderClapModifier.Effects.get(0).TriggerSpell.set(Consecrationdamage.ID);
ThunderClapModifier.Effects.get(0).ImplicitTargetA.UNIT_TARGET_ENEMY.set();
...
export let FelFlamestrike = std.Spells.create(
    MODNAME,
    "felflamestrike-spell",
    39139
);
...
export let FuryHammerEffect = std.Spells.create(
    MODNAME,
    "furyhammereffect-spell",
    61576
);
