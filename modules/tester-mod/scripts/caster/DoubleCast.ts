...
export function DoubelCast(events: TSEventHandlers) {
    ...
    events.SpellID.OnCast(spellID,(spell)=>{
        ...
        let flag = caster.GetData().GetObject<PlayerDoublecast>(ModID(),TABLE_NAME_DOUBLE,()=>new PlayerDoublecast()).flag
        if(flag == 0){
            ...
            caster.GetTasks().AddTimer(ModID(),"reset_doublecast",mineDespawn,1,(timer,entity,del,can)=>{
                entity.ToPlayer().GetData().GetObject<PlayerDoublecast>(ModID(),TABLE_NAME_DOUBLE).flag = 0
            });
        }
        else{
