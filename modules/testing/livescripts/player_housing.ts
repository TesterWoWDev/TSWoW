    ...
    events.Spells.OnCheckCast((spell) => {
        if (spell.GetSpellInfo().SchoolMask() == 98) {
            ...
            if (player.GetMap().GetUInt("playerOwner", 1) != player.GetGUIDLow()) {
                ...
                let q = QueryWorld(
                    "SELECT gobID FROM `player_housing_item_spell_link` WHERE spellID = " +
                    spell.GetEntry()
                );
                while (q.GetRow()) {
                    QueryCharacters(
                        "INSERT INTO `player_housing` VALUES(" +
                        player.GetGUIDLow() +
                        "," +
                        q.GetUInt32(0) +
                        "," +
                        spell.GetTargetDest().x +
                        "," +
                        spell.GetTargetDest().y +
                        "," +
                        spell.GetTargetDest().z +
                        "," +
                        spell.GetTargetDest().o +
                        ")"
                    );
                }
            }
            ...
            if (gobs.length > 0) {
                ...
                for (let i = 0; i < gobs.length; i++) {
                    player.GossipMenuAddItem(
                        0,
                        "Remove " + gobs[i].GetName() + " ID:" + gobs[i].GetGUIDLow(),
                        0,
                        gobs[i].GetGUIDLow(),
                        false,
                        "",
        ...
        (obj, player, menuID, selection, cancel) => {
            if (player.GetMapId() == mapID) {
                if (player.GetMap().GetUInt("playerOwner", 1) != player.GetGUIDLow()) {
                    ...
                    let gobs: TSArray<TSGameObject> = player.GetGameObjectsInRange(
                        5,
                        0,
                        0
                    );
                    ...
                    if (index == -1) {
                        ...
                        gobs[index].Despawn();
                        QueryCharacters(
                            "DELETE FROM `player_housing` WHERE x = " +
                            pos.x +
                            " AND y = " +
                            pos.y +
                            " AND z = " +
